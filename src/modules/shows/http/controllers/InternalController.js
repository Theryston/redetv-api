...
module.exports = {
    createNewLogo: async (req, res) => {
        try {
            ...
            const newLogo = await InternalService.createNewLogo({ link: datas.link, company_name: datas.company_name, active: true, key })
            res.json(newLogo);
        } catch (error) {
            console.error(error)
    ...
    getAllNewLogos: async (req, res) => {
        try {
            ...
            for (const logo of logos) {
                ...
                logosWithUrl.push({ link: logo.link, company_name: logo.company_name, url: url, _id: logo._id });
            }
    ...
    getNewLogo: async (req, res) => {
        try {
            ...
            const url = await ShowService.getDownloadUrl(logo.key);
            res.json({ link: logo.link, company_name: logo.company_name, url: url, _id: logo._id })
        } catch (error) {
            res.status(500).json({ error })
        }
    },
    ...
    updateNewLogo: async (req, res) => {
        try {
            ...
            if (req.file) {
                const key = await ShowService.uploadToOnedrive(req.file.buffer, datas.folder, { fileSize: req.file.size, filename: Date.now().toString() + '-' + req.file.originalname });
                const logo = await InternalService.updateNewLogo(logo_id, { ...datas, key })
                res.json(logo);
            } else {
